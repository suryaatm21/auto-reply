(() => {
  ...
  const CONFIG = {
    ...
    keywords: ['systems'],
    maxRepliesPerRun: 2000, // fast test cap
    ...
    delays: {
      betweenActionsMs: [312, 615], // was [4000, 9000]
      betweenRepliesMs: [969, 5557], // was [15000, 30000]
      afterLoadMoreMs: [248, 496], // was [1500, 3000]
      typingPerCharMs: [16, 45], // was [25, 60]
    },
    dryRun: false,
    ...
    editorSettleMs: [255, 478], // short settle after typing
  ...
  function saveProgress(p) {
    try {
      localStorage.setItem(getPostKey(), JSON.stringify(p));
    } catch {}
  }
  ...
  function findReplyButton(el) {
    ...
    for (const s of sels) {
      ...
      for (const b of btns) {
        ...
        if (
          b.offsetParent !== null &&
          (aria.includes('reply') || txt === 'reply' || txt.includes('reply'))
        )
          return b;
      }
    }
  ...
  function findLikeButton(el) {
    const candidates = el.querySelectorAll(
      'button[aria-label*="Like" i], button[aria-label*="React Like" i], button[aria-pressed]',
    );
  ...
  function hasOwnReply(el) {
    const blocks = el.querySelectorAll(
      '[data-test-id*="reply"], .comments-comment-item--reply, [id*="reply-"]',
    );
    for (const b of blocks) {
      if (
        b.querySelector(
          '[aria-label^="You"], .comments-comment-item__main-content--own',
        )
      )
        return true;
      const meta = b.querySelector(
        '[data-test-reusable-actor__meta], [data-test-reusable-actor__name]',
      );
  ...
  async function replyAndMaybeLike(commentEl, key) {
    try {
      ...
      if (!editor) {
        log('Reply editor not found', 'error');
        return false;
      }
      ...
      if (!submitReady) {
        ...
        if (!submit) {
          log('Submit/Reply button not found near editor', 'error');
          return false;
        }
        if (CONFIG.dryRun) {
          submit.style.outline = '2px solid lime';
          log('DRY-RUN: would click Submit');
        } else {
          submit.click();
        }
        await delay(CONFIG.delays.betweenActionsMs);
      ...
      if (CONFIG.likeAfterReply) {
        ...
        if (likeBtn && !isLiked(likeBtn)) {
          if (CONFIG.dryRun) {
            likeBtn.style.outline = '2px solid magenta';
            log('DRY-RUN: would click Like');
          } else {
            likeBtn.click();
          }
          await sleep(250);
      ...
      // Persist
      if (
        !CONFIG.requireLikeAsProcessed ||
        (CONFIG.requireLikeAsProcessed && isLiked(findLikeButton(commentEl)))
      ) {
        persistKey(key);
      }
  ...
  async function process() {
    ...
    try {
      ...
      while (
        !shouldStop &&
        repliedThisRun < CONFIG.maxRepliesPerRun &&
        scrollChunks < CONFIG.scroll.maxScrollChunks
      ) {
        ...
        if (!comments.length) {
          log('No comments visible yet… scrolling.', 'info');
        ...
        for (
          ...
          if (CONFIG.skipIfAlreadyReplied && hasOwnReply(c)) {
            persistKey(key);
            continue;
          }
          ...
          if (ok) {
            repliedThisRun++;
            log(
              `Replied ${repliedThisRun}/${CONFIG.maxRepliesPerRun}`,
              'success',
            );
      ...
      log(
        `Run complete. Replied this run: ${repliedThisRun}. Total processed: ${processedSet.size}`,
        'success',
      );
    } catch (e) {
  ...
  window.liReply = {
    ...
    stop() {
      shouldStop = true;
      paused = false;
      log('Stop requested; exiting ASAP');
    },
    setMax(n) {
      if (Number.isFinite(n) && n > 0) {
        CONFIG.maxRepliesPerRun = n;
        log(`maxRepliesPerRun set to ${n}`);
      }
    },
    setKeywords(arr) {
      CONFIG.keywords = Array.isArray(arr) ? arr : [String(arr)];
  ...
  // ================== START ==================
  log(
    'Loaded. Controls: liReply.start() / liReply.pause() / liReply.stop() / liReply.setMax(n) / liReply.setKeywords([...]) / liReply.stats()  (P=Pause, S=Stop)',
  );
  log(
    `Target keywords: ${CONFIG.keywords.join(', ')}  |  Per-run cap: ${
