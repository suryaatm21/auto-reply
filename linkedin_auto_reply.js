(() => {
  ...
  const CONFIG = {
    ...
    minDelay: 3000,
    maxDelay: 8000,
    maxRepliesPerRun: 70, // per-run throttle; NOT a hard limit overall
  ...
  const randomTemplate = () =>
    CONFIG.messageTemplates[rand(0, CONFIG.messageTemplates.length - 1)];
  const log = (m, t = 'info') => {
  ...
  function saveProgress(progress) {
    try {
      localStorage.setItem(getPostKey(), JSON.stringify(progress));
    } catch {}
  }
  ...
  async function replyToComment(commentEl, message) {
    try {
      ...
      if (CONFIG.dryRun) {
        ...
        if (!submit) {
          log('Submit/Reply button not found', 'error');
          return false;
        }
        submit.click();
      }
  ...
  async function process() {
    ...
    try {
      ...
      for (
        let i = 0;
        i < comments.length && repliedThisRun < CONFIG.maxRepliesPerRun;
        i++
      ) {
        await waitWhilePaused();
        ...
        if (ok) {
          ...
          const d = randomDelay();
          log(
            `Replied (${repliedThisRun}/${
              CONFIG.maxRepliesPerRun
            }). Cooling down ${(d / 1000).toFixed(1)}s…`,
            'success',
          );
          await sleep(d);
      ...
      log(
        `Run complete. Replied this run: ${repliedThisRun}. Total processed (saved): ${processedSet.size}`,
        'success',
      );
    } catch (e) {
  ...
  window.liReply = {
    ...
    stop() {
      running = false;
      paused = false;
      log('Stop requested; will exit after current step');
    },
    setMax(n) {
  ...
  // ---------- START ----------
  log(
    'Loaded. Controls: liReply.pause() / liReply.stop() / liReply.setMax(n) / liReply.stats() / liReply.reset()  (P=Pause, S=Stop)',
  );
  log(
    `Delay: ${CONFIG.minDelay / 1000}-${
      CONFIG.maxDelay / 1000
    }s | Per-run cap: ${
      CONFIG.maxRepliesPerRun
    } (raise with liReply.setMax(n))`,
  );
